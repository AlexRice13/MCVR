...
constexpr std::string_view DEFAULT_SCENARIO_NAME = "Default";
...
ToneMappingSettings applyScenarioValues(const ToneMappingSettings &baseSettings,
...
std::string boolToPipelineValue(bool value) {
    return value ? "render_pipeline.true" : "render_pipeline.false";
}

std::string toneMappingMethodToPipelineValue(int value) {
    switch (value) {
        case TONE_MAPPING_METHOD_REINHARD: return "render_pipeline.module.tone_mapping.attribute.method.reinhard";
        case TONE_MAPPING_METHOD_REINHARD_WHITE_POINT:
            return "render_pipeline.module.tone_mapping.attribute.method.reinhard_white_point";
        case TONE_MAPPING_METHOD_ACES_FITTED: return "render_pipeline.module.tone_mapping.attribute.method.aces";
        case TONE_MAPPING_METHOD_ACES_FITTED_WHITE_POINT:
            return "render_pipeline.module.tone_mapping.attribute.method.aces_white_point";
        case TONE_MAPPING_METHOD_UNCHARTED2: return "render_pipeline.module.tone_mapping.attribute.method.uncharted2";
        case TONE_MAPPING_METHOD_PBR_NEUTRAL:
        default: return "render_pipeline.module.tone_mapping.attribute.method.pbr_neutral";
    }
}

std::string exposureMeteringModeToPipelineValue(int value) {
    switch (value) {
        case TONE_MAPPING_EXPOSURE_METERING_MODE_CENTER:
            return "render_pipeline.module.tone_mapping.attribute.exposure_metering_mode.center";
        case TONE_MAPPING_EXPOSURE_METERING_MODE_GLOBAL:
        default: return "render_pipeline.module.tone_mapping.attribute.exposure_metering_mode.global";
    }
}

std::optional<std::string> exportSettingValue(const ToneMappingSettings &settings, const std::string &key) {
    if (key == "render_pipeline.module.tone_mapping.attribute.middle_grey") return formatFloatValue(settings.middleGrey);
    if (key == "render_pipeline.module.tone_mapping.attribute.exposure_up_speed") return formatFloatValue(settings.speedUp);
    if (key == "render_pipeline.module.tone_mapping.attribute.exposure_down_speed") return formatFloatValue(settings.speedDown);
    if (key == "render_pipeline.module.tone_mapping.attribute.log2_luminance_min") return formatFloatValue(settings.log2Min);
    if (key == "render_pipeline.module.tone_mapping.attribute.log2_luminance_max") return formatFloatValue(settings.log2Max);
    if (key == "render_pipeline.module.tone_mapping.attribute.histogram_epsilon") return formatFloatValue(settings.epsilon);
    if (key == "render_pipeline.module.tone_mapping.attribute.low_percent") return formatFloatValue(settings.lowPercent);
    if (key == "render_pipeline.module.tone_mapping.attribute.high_percent") return formatFloatValue(settings.highPercent);
    if (key == "render_pipeline.module.tone_mapping.attribute.min_exposure") return formatFloatValue(settings.minExposure);
    if (key == "render_pipeline.module.tone_mapping.attribute.max_exposure") return formatFloatValue(settings.maxExposure);
    if (key == "render_pipeline.module.tone_mapping.attribute.enable_auto_exposure") return boolToPipelineValue(settings.autoExposure);
    if (key == "render_pipeline.module.tone_mapping.attribute.manual_exposure") return formatFloatValue(settings.manualExposure);
    if (key == "render_pipeline.module.tone_mapping.attribute.exposure_bias") return formatFloatValue(settings.exposureBias);
    if (key == "render_pipeline.module.tone_mapping.attribute.white_point") return formatFloatValue(settings.whitePoint);
    if (key == "render_pipeline.module.tone_mapping.attribute.saturation") return formatFloatValue(settings.saturation);
    if (key == "render_pipeline.module.tone_mapping.attribute.contrast") return formatFloatValue(settings.contrast);
    if (key == "render_pipeline.module.tone_mapping.attribute.gamma") return formatFloatValue(settings.gradingGamma);
    if (key == "render_pipeline.module.tone_mapping.attribute.lift") return formatFloatValue(settings.lift);
    if (key == "render_pipeline.module.tone_mapping.attribute.gain") return formatFloatValue(settings.gain);
    if (key == "render_pipeline.module.tone_mapping.attribute.temperature") return formatFloatValue(settings.temperature);
    if (key == "render_pipeline.module.tone_mapping.attribute.tint") return formatFloatValue(settings.tint);
    if (key == "render_pipeline.module.tone_mapping.attribute.clamp_output") return boolToPipelineValue(settings.clampOutput);
    if (key == "render_pipeline.module.tone_mapping.attribute.method")
        return toneMappingMethodToPipelineValue(settings.toneMappingMethod);
    if (key == "render_pipeline.module.tone_mapping.attribute.exposure_metering_mode")
        return exposureMeteringModeToPipelineValue(settings.exposureMeteringMode);
    if (key == "render_pipeline.module.tone_mapping.attribute.center_metering_percent")
        return formatFloatValue(settings.centerMeteringPercent);
    return std::nullopt;
}
...
ScenarioResolvedState ScenarioColorGradingManager::resolveState(const ToneMappingSettings &baseSettings, bool hdrActive) {
    std::scoped_lock lock(mutex_);
    ensureLoadedLocked();
    return resolveStateLocked(baseSettings, hdrActive);
}

ToneMappingSettings ScenarioColorGradingManager::resolveSettings(const ToneMappingSettings &baseSettings, bool hdrActive) {
    std::scoped_lock lock(mutex_);
    ensureLoadedLocked();
    return resolveStateLocked(baseSettings, hdrActive).settings;
}

std::vector<std::string> ScenarioColorGradingManager::exportResolvedAttributePairs(
    const ToneMappingSettings &baseSettings,
    bool hdrActive,
    const std::vector<std::string> &attributeKeys) {
    std::scoped_lock lock(mutex_);
    ensureLoadedLocked();
    ScenarioResolvedState state = resolveStateLocked(baseSettings, hdrActive);
    std::vector<std::string> pairs;
    pairs.reserve(attributeKeys.size() * 2);
    for (const auto &key : attributeKeys) {
        auto value = exportSettingValue(state.settings, key);
...
ScenarioResolvedState ScenarioColorGradingManager::resolveStateLocked(const ToneMappingSettings &baseSettings, bool hdrActive) {
    if (hasPreviewSettings_) {
        return {
            .displayName = "Preview",
            .sourceName = "Preview",
            .targetName = "Preview",
            .settings = previewSettings_,
            .transitioning = false,
        };
    }

    ToneMappingSettings resolvedSettings = baseSettings;
    const ScenarioDefinition *scenario = findBestMatchLocked();
    std::string resolvedName = scenario == nullptr ? std::string(DEFAULT_SCENARIO_NAME) : scenario->name;
    if (scenario != nullptr) {
        const auto &values = hdrActive ? scenario->hdrOnValues : scenario->hdrOffValues;
        resolvedSettings = applyScenarioValues(baseSettings, values);
    }
