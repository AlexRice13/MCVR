...
bool isHdrOutputFormat(VkFormat format) {
    switch (format) {
...
void ToneMappingModule::setAttributes(int attributeCount, std::vector<std::string> &attributeKVs) {
    ...
    isAutoExposureEnabled_ = settings.autoExposure;
    shouldClampOutput_ = settings.clampOutput;
...
ToneMappingSettings ToneMappingModule::captureBaseSettings() const {
    ...
    settings.autoExposure = isAutoExposureEnabled_;
    settings.clampOutput = shouldClampOutput_;
...
void ToneMappingModuleContext::render() {
    ...
    pc.gradingGamma = std::max(resolvedSettings.gradingGamma, 1e-6f);
    ...
    pc.hdrActive = (Renderer::options.hdrActive && isHdrOutputFormat(ldrImage->vkFormat())) ? 1 : 0;
