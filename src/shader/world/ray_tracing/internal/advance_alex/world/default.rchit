...
void sampleSurfaceState(bool useTexture,
    ...
    alexApplyRainMaterial(packedData, skyUBO.rainGradient, worldPos, uv, textureID, mat, albedoValue);
    ...
    if (isFftWaterSurface && !localHit.sideWall) {
        ...
        if (!localHit.sideWall) {
            ...
            if (alexHasRainAnisotropicMaterial(packedData)) {
                shadingNormal = alexBuildPrecipitationNormal(shadingNormal, viewDir, worldPos, uv, textureID);
            }
        }
    }
...
vec3 sampleSurfaceDirectionalLight(SampledSurface surface,
    ...
    vec3 sampledLightDir =
        SampleVMF(mainRay.seed, lightDir, 1.0 / max(skyUBO.sunAngularRadius * skyUBO.sunAngularRadius, 1e-6));
    ...
    if (ADV_CLOUD_MODE == 2u) {
        ...
        lightRadiance *= alexBlockyCloudLightVisibility(absoluteShadowOriginPos, lightDir);
    }
