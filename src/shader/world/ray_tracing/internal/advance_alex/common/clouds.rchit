...
float lightMarchOD(vec3 p, vec3 lightDir, float opacity, float densityGrad, vec2 windOffset, float edgeSoftness) {
...
vec3 multiScatterStep(float od, float lightOD, float cosTheta, float anisotropy, vec3 sunColor, vec3 skyLight) {
...
vec3 alexCloudLightDirection() {
    vec3 sunDir = celestialSunDirection();
    return sunDir.y >= 0.0 ? sunDir : -sunDir;
}
...
void main() {
    ...
    float edgeSoftness = max(skyUBO.cloudEdgeSoftness, 1e-3);
    vec2 windOffset = vec2(skyUBO.cloudWindOffsetX, skyUBO.cloudWindOffsetZ);

    vec3 lightDir = alexCloudLightDirection();
    ...
    traceRayEXT(topLevelAS, gl_RayFlagsNoneEXT, WORLD_MASK, 0, 0, 0, cloudShadowOrigin,
                0.001, lightDir, 10000.0, 1);
    ...
    for (int i = 0; i < steps; i++, marchPos += increment) {
        ...
        float coverage = sampleCloudCoverage(marchPos.xz + windOffset, edgeSoftness);
        float density = coverage * mix(1.0, vertProfile, densityGrad) * opacity * CLOUD_DENSITY_SCALE;

        float od = stepLength * density;
        if (od <= 0.0) { continue; }

        float lightOD = lightMarchOD(marchPos, lightDir, opacity, densityGrad, windOffset, edgeSoftness);
        vec3 stepScatter = multiScatterStep(od, lightOD, lDotW, anisotropy, sunColor, skyLight);
    ...
    rayStoreMaterial(mainRay, vec4(1.0, 1.0, 1.0, cloudAlpha), vec3(0.04), 1.0, 0.0, 0.0, 1.5, 0.0);
