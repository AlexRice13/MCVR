...
vec3 sampleSkyAtmosphereTransmittance(float r, float mu) {
    return sampleTexture(transLUT, atmosphereTransmittanceUv(r, mu), false).rgb;
}
...
vec4 evalSunBillboard(vec3 rayDir) {
    ...
    vec2 q = p / max(z, 1e-4);
    float tanHalf = tan(max(skyUBO.sunAngularRadius, 0.001));
    vec2 a = abs(q);
...
vec4 evalMoonBillboard(vec3 rayDir) {
    ...
    vec2 p = vec2(dot(rayDir, right), dot(rayDir, up));
    vec2 q = p / max(z, 1e-4);
    float tanHalf = tan(max(skyUBO.sunAngularRadius * 1.667, 0.001667));
    vec2 a = abs(q);
    if (a.x > tanHalf || a.y > tanHalf) return vec4(0.0);
    vec2 uv = q / tanHalf * 0.5 + 0.5;
...
void main() {
    ...
    if (worldUBO.skyType == 1) {
        ...
        vec3 transmittance = sampleSkyAtmosphereTransmittance(r, mu);
